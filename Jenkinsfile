...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="hayangom92/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/hayangom92/guestbook.git'
            }
        }
        stage('Build') {
            steps {
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_hayangom92') {
                        oDockImage.push()
                    }
                }
            }
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'hayangom92.j@gmail.com')
